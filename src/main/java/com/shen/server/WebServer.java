...
public class WebServer {
	 ...
	 *
	 * @param webappPath
	 *            webapp的路径，通常包含了JSP和WEB-INF/web.xml的文件路径
	 * @param port
	 *            端口
	 * @param contextPath
	 *            监听的servelt的context path
	 * @return jetty服务器实例
	 */
	public static Server createServer(String webappPath, int port,
			String contextPath) {
	...
	public static Server createTapestryWebapp(int port, String pkg,
		...
		try {
			filterHolder = new FilterHolder((Class<? extends Filter>) Thread
					.currentThread().getContextClassLoader()
					.loadClass("org.apache.tapestry5.TapestryFilter"));
	...
	public static Server createTapestryWebapp(int port, String pkg,
		...
		try {
			filterHolder = new FilterHolder((Class<? extends Filter>) Thread
					.currentThread().getContextClassLoader().loadClass(filter));
		} catch (ClassNotFoundException e) {
			throw new RuntimeException(e);
		}
		filterHolder.setName(appName);
	 ...
	 * @return jetty服务器实例
	 */
	public static Server createServer(String webXmlPath, String webappPath,
